    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <div className="relative h-[70vh] md:h-[80vh] flex items-end">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 pb-14 w-full">
          ...
          <p className="text-white/50 text-xs font-medium tracking-[0.2em] uppercase mb-3 flex items-center gap-1.5">
        ...
        {user && (
          <Link
            ...
            className="absolute top-20 right-6 flex items-center gap-2 bg-white/15 backdrop-blur-sm hover:bg-white/25 text-white text-xs font-medium px-4 py-2 rounded-full transition-colors border border-white/20"
          >
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="lg:grid lg:grid-cols-[1fr_260px] lg:gap-16">
          ...
          <div>
            {guide.intro && (
              <p className="font-serif text-xl md:text-2xl text-warm-600 leading-relaxed mb-16 italic">
                « {guide.intro} »
              </p>
            )}
          ...
          {sections.length > 0 && (
            <aside className="hidden lg:block">
              <div className="sticky top-24 bg-cream-100 border border-cream-200 rounded-2xl p-6">
                ...
                <ul className="space-y-2.5">
                  {Object.entries(typeCounts).map(([type, count]) => (
                    <li key={type} className="flex items-center justify-between text-sm text-warm-500">
                      ...
                      <span className="bg-white border border-cream-200 rounded-full px-2 py-0.5 text-xs font-semibold text-warm-400">{count}</span>
                    </li>
                  ))}
                </ul>
